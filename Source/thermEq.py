...
def thermEq(temp, run, newDensity, pdb_file, cudaIndex, saveFolder):
    ##########################################################################
    # Variables
    ##########################################################################
    ...
    print('Final minimized T =', simulation.context.getState(getEnergy=True).getKineticEnergy() / (0.5 * DOF * BOLTZ))
